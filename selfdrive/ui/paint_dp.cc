...
void ui_draw_fd_button(UIState *s) {
  nvgBeginPath(s->vg);
  ...
  s->scene.dpFollowingProfile == 3? "1.5s" :
  s->scene.dpFollowingProfile == 2? "1.2s" :
  s->scene.dpFollowingProfile == 1? "0.9s" :
  "1.8s",
  NULL);
...
void ui_draw_infobar(UIState *s) {
  ...
  nvgFillColor(s->vg, (s->scene.brakeLights? COLOR_RED_ALPHA(200) : COLOR_BLACK_ALPHA(100)));
  nvgFill(s->vg);
...
void bb_ui_draw_measures_left(UIState *s, int bb_x, int bb_y, int bb_w ) {
  ...
  if (true) {
    ...
    bb_h +=bb_ui_draw_measure(s,  val_str, uom_str,
        (s->scene.dpLocale == "zh-TW"? "相對速度" : s->scene.dpLocale == "zh-CN"? "相对速度" : "REL SPEED"),
        bb_rx, bb_ry, bb_uom_dx,
        val_color, lab_color, uom_color,
        value_fontSize, label_fontSize, uom_fontSize );
    bb_ry = bb_y + bb_h;
