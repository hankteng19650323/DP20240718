...
class LongitudinalMpc:
  ...
  def update_TF(self, carstate):
    ...
    elif carstate.distanceLines == 3: # Roadtrip 
      x_vel = [0, 2.788, 5.56, 8.333, 11.11, 13.89, 16.67, 25.0, 41.67]  # velocities
      y_dist = [1.27, 1.27, 1.29, 1.32, 1.38, 1.38, 1.38, 1.619, 1.8]      
      self.desired_TF = np.interp(carstate.vEgo, x_vel, y_dist)
      #self.desired_TF = T_FOLLOW
      
    else: # Stock or Relaxed
      x_vel = [0, 2.788, 5.56, 8.333, 11.11, 13.89, 16.67, 25.0, 41.67]  # velocities
      y_dist = [1.24, 1.24, 1.27, 1.29, 1.35, 1.35, 1.35, 1.38, 1.45]
      self.desired_TF = np.interp(carstate.vEgo, x_vel, y_dist)
