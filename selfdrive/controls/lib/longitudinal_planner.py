...
DP_FOLLOWING_OFF = 0
DP_FOLLOWING_SHORT = 1
DP_FOLLOWING_NORMAL = 2
DP_FOLLOWING_LONG = 3
...
class Planner():
  def __init__(self, CP):
    ...
    # dp
    self.dp_accel_profile = DP_ACCEL_OFF
    self.dp_follow_profile = DP_FOLLOWING_OFF
  ...
  def update(self, sm, CP):
    ...
    self.dp_follow_profile = sm['dragonConf'].dpFollowingProfile
    dp_following_dist = DP_FOLLOWING_DIST[self.dp_follow_profile]
    ...
    if enabled and not self.first_loop and not pedal_pressed:
      if self.dp_accel_profile == DP_ACCEL_OFF:
