...
def get_lkas_cmd_bus(car_fingerprint, radar_disabled=False, has_relay=False):
  if radar_disabled:
...
def create_brake_command(packer, apply_brake, pump_on, pcm_override, pcm_cancel_cmd, fcw, idx, car_fingerprint, stock_brake, has_relay=False):
...
def create_steering_control(packer, apply_steer, lkas_active, car_fingerprint, idx, radar_disabled, has_relay=False):
  values = {
    "STEER_TORQUE": apply_steer if lkas_active else 0,
    "STEER_TORQUE_REQUEST": lkas_active,
  }
  bus = get_lkas_cmd_bus(car_fingerprint, radar_disabled, has_relay)
...
def create_ui_commands(packer, pcm_speed, hud, car_fingerprint, is_metric, idx, openpilot_longitudinal_control, stock_hud, has_relay=False):
  commands = []
  ...
  bus_lkas = get_lkas_cmd_bus(car_fingerprint, radar_disabled, has_relay)
